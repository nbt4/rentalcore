...
// EnsureJobStatusLifecycle consolidates all historic status variants into the
// four canonical job lifecycle states. It is intentionally idempotent because
// production deployments do not use a separate migration runner.
func EnsureJobStatusLifecycle(db *sql.DB) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("begin job status migration: %w", err)
	}
	defer tx.Rollback()
	...
	for _, statement := range statements {
		if _, err := tx.Exec(statement); err != nil {
			return fmt.Errorf("apply job status migration: %w", err)
		}
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit job status migration: %w", err)
	}
	logger.LogInfo("job status schema: canonical lifecycle verified")
	return nil
}
