...
func (h *AnalyticsHandler) getSimplifiedJobs(startDate, endDate time.Time) map[string]interface{} {
	...
	// Count completed jobs using the canonical lifecycle status.
	h.db.Raw(`
		SELECT COUNT(*) 
		FROM jobs 
		WHERE endDate BETWEEN ? AND ? 
		AND statusID = ?
	`, startDate, endDate, jobstatus.CompletedID).Scan(&completedJobs)

	// Planning and confirmed jobs are open.
	h.db.Raw(`
		SELECT COUNT(*) 
		FROM jobs 
		WHERE startDate <= ? AND (endDate >= ? OR endDate IS NULL) 
		AND statusID IN ?
	`, endDate, startDate, jobstatus.OpenIDs).Scan(&activeJobs)
...
func (h *AnalyticsHandler) getDeviceAnalyticsData(deviceID string, startDate, endDate time.Time, period string) map[string]interface{} {
	...
	for _, booking := range customerBookings {
		switch strings.ToLower(strings.TrimSpace(booking.JobStatus)) {
		case strings.ToLower(jobstatus.Completed):
			statusCounts[jobstatus.Completed]++
		case strings.ToLower(jobstatus.Cancelled):
			statusCounts[jobstatus.Cancelled]++
		case strings.ToLower(jobstatus.Planning), strings.ToLower(jobstatus.Confirmed):
			statusCounts["Offen"]++
		}
	}
	...
	return map[string]interface{}{
		...
		"statusDistribution": map[string]interface{}{
			"labels": []string{jobstatus.Completed, "Offen", jobstatus.Cancelled},
			"data":   []int{statusCounts[jobstatus.Completed], statusCounts["Offen"], statusCounts[jobstatus.Cancelled]},
		},
...
func (h *AnalyticsHandler) getJobAnalytics(startDate, endDate time.Time) map[string]interface{} {
	...
	// Completed jobs
	h.db.Model(&models.Job{}).
		Where("endDate BETWEEN ? AND ? AND statusID = ?", startDate, endDate, jobstatus.CompletedID).
		Count(&completedJobs)

	// Active jobs
	h.db.Model(&models.Job{}).
		Where("startDate <= ? AND (endDate >= ? OR endDate IS NULL) AND statusID IN ?",
			endDate, startDate, jobstatus.OpenIDs).
		Count(&activeJobs)

	// Overdue jobs
	h.db.Model(&models.Job{}).
		Where("endDate < ? AND statusID IN ?", time.Now(), jobstatus.OpenIDs).
		Count(&overdueJobs)
